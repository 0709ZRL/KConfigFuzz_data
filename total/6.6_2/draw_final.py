    ...
    for i, (x, y) in enumerate(data):
        ...
        x_sm, y_sm = smooth_xy(
            ...
            ma_minutes=MA_MINUTES
                ...
                # 仅对红线做强力平滑（如果你已按上一版添加了 strong_smooth_monotone，可保留）
        ...
        # NEW: 如果 HEALER 不到 24 小时，用最后一个点水平填充到 24 小时
        if lab == 'HEALER' or ('healer' in lab.lower()):
            if len(x_sm) > 0 and x_sm[-1] < H_CUTOFF:
                step_h = RESAMPLE_STEP_MIN / 60.0  # 与重采样步长一致（小时）
                x_pad = np.arange(x_sm[-1] + step_h, H_CUTOFF + 1e-12, step_h)
                if x_pad.size > 0:
                    y_pad = np.full(x_pad.shape[0], y_sm[-1], dtype=float)
                    x_sm = np.concatenate([x_sm, x_pad])
                    y_sm = np.concatenate([y_sm, y_pad])
